...
@Component({
  ...
  templateUrl: './antecedente-patologico.page.html',
  styleUrls: ['./antecedente-patologico.scss'],
})
export class AntecedentePatologicoPage implements OnInit {
  ...
  ngOnInit() {
    this.activatedRoute.params.pipe(
      switchMap( ({id}) => {
        this.idPaciente = id;
        return this.antecedentePatologicoService.obtener(id);
      })
    ).subscribe( resp => {
      console.log(resp);
      this.datas = resp;
  ...
  obtenerDatos() {
    this.antecedentePatologicoService.obtener(this.idPaciente).subscribe( (resp) => {
      this.datas = resp;
    });
  }
  async presentModal() {
  ...
  async presentActionSheet(data: Antecedente) {
    const actionSheet = await this.actionSheetController.create({
      ...
      buttons: [{
        ...
        icon: 'trash',
        handler: () => {
          this.antecedentePatologicoService.borrar(data.id).subscribe( resp => {
            this.obtenerDatos();

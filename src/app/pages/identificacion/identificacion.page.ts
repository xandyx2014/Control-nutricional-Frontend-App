...
export class IdentificacionPage implements OnInit {
  ...
  ngOnInit() {
    this.activatedRoute.params.pipe(
      switchMap( ({id}) => {
        this.idPaciente = id;
        return this.identificacionService.obtener(id);
      })
    ).subscribe( resp => {
      console.log(resp);
      this.datas = resp;
  ...
  obtenerDatos() {
    this.identificacionService.obtener(this.idPaciente).subscribe( (resp) => {
      this.datas = resp;
    });
  }
  async presentModal() {
  ...
  async presentActionSheet(data: Antecedente) {
    const actionSheet = await this.actionSheetController.create({
      ...
      buttons: [{
        ...
        icon: 'trash',
        handler: () => {
          this.identificacionService.borrar(data.id).subscribe( resp => {
            this.obtenerDatos();
